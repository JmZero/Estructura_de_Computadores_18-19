...
int popcount1(unsigned* array, size_t len){
  ...
  for(size_t j = 0; j < len; j++){                 //Para recorrer los valores de la lista
    x = array[j];
    for (size_t i = 0; i < 8*sizeof(int); i++) {   //Recorre cada bit de la componente
      result += x & 0x1;
      x >>= 1;
    }
  }
...
int popcount2(unsigned* array, size_t len){
  ...
  for(size_t j = 0; j < len; j++){                 //Para recorrer los valores de la lista
    x = array[j];
    while(x){                                      //Mientras que la componente no sea 0
      result += x & 0x1;
      x >>= 1;                                     //Desplazamos x a la derecha (dividimos entre 2)
    }
  }
  return result;
}
...
int popcount4(unsigned* array, size_t len){
  ...
  for(size_t j = 0; j < len; j++){                 //Para recorrer los valores de la lista
    ...
    "ini4:                \n\t"
          "adc $0, %[r]   \n\t"                 // Sumamos si hay acarreo en este caso por la derecha
          "shr     %[x]   \n\t"                 // Desplaza 1 hacia la derecha, esta dentro de un bucle ini4
          "jnz ini4       \n\t"
    "fin4:                \n\t"
          "adc $0, %[r]   \n\t"
          :[r]"+r" (result)                     // Indica que r es de lectura-escritura desde result, r referencia a result en c
...
for (i=0; i<len; i+=4){
  asm("movdqu   %[x],   %%xmm0  \n\t"
      ...
      "movdqu   %[l],   %%xmm2  \n\t"     //since instruction phsufb modifies LUT
      "movdqa   %%xmm2, %%xmm3  \n\t"     //we need 2 copies
      ...
      "paddd    %%xmm3, %%xmm0  \n\t"     //xmm3 = [  not needed  |pcnt bytes0..15]
      "movd     %%xmm0, %[val]      "
      : [val]"=r" (val)
      : [x]  "m"  (array[i]),
        [m]  "m"  (SSE_mask[0]),
